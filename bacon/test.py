...
def _run_pickled_together_test(n):
    ...
    for _ in range(10_000):
        for a1, a2, v in tests:
            ...
            # I will ask the prof about this part later
...
def tiny_test_bacon_number_0():
    # Actors with Bacon number of 0
    assert(lab.actors_with_bacon_number(db_tiny, 0)) == {4724}

def tiny_test_bacon_number_1():
    # Actors with Bacon number of 1
    assert(lab.actors_with_bacon_number(db_tiny, 1)) == {1532, 2876}

def tiny_test_bacon_number_2():
    # Actors with Bacon number of 2
    assert(lab.actors_with_bacon_number(db_tiny, 2)) == {1640}

def tiny_test_bacon_number_3():
    # Actors with Bacon number of 3
...
def tiny_test_bacon_path():
    # Bacon path, tiny database, length of 3 (3 actors, 2 movies)
    actor_id = 1640
    len_expected = 3
...
def tiny_test_actor_to_actor_path():
    # actor path that exists
    actor_1 = 1640
    actor_2 = 1532
    # 1640 -> 2876 -> 1532 / 1640 -> 4724 -> 1532
    len_expected = 2
    res1 = lab.actor_to_actor_path(db_tiny, actor_1, actor_2)
    res2 = lab.actor_to_actor_path(db_tiny, actor_1, actor_2)
    res3 = lab.actor_to_actor_path(db_tiny, actor_1, actor_2)

    check_valid_path(fset_tiny, res1, actor_1, actor_2, len_expected)
    check_valid_path(fset_tiny, res2, actor_1, actor_2, len_expected)
    check_valid_path(fset_tiny, res3, actor_1, actor_2, len_expected)
