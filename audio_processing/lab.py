...
def echo(sound, num_echoes, delay, scale):
    ...
    sam = sound["samples"]
    rate = sound["rate"]
    delay_sam = round(delay * rate)
    res_len = len(sam) + delay_sam * num_echoes
    res_sam = [0] * res_len
    for i in range(len(sam)):
        res_sam[i] += sam[i]

    for e in range(1, num_echoes + 1):
        for i in range(len(sam)):
            id = i + e * delay_sam
            if id < res_len:
                res_sam[id] += sam[i] * (scale**e)
    return {"rate": rate, "samples": res_sam}
...
def pan(sound):
    ...
    sam_r = sound["right"]
    num_sam = len(sam_l)
    res_l = [sam * (1 - (i / (num_sam - 1))) for i, sam in enumerate(sam_l)]
    res_r = [sam * (i / (num_sam - 1)) for i, sam in enumerate(sam_r)]
    return {"rate": sound["rate"], "left": res_l, "right": res_r}
...
def remove_vocals(sound):
    ...
    sam_r = sound["right"]
    num_sam = len(sam_l)
    res_sam = [(sam_l[i] - sam_r[i]) for i in range(num_sam)]
    return {"rate": sound["rate"], "samples": res_sam}
...
if __name__ == "__main__":
    ...
    # write_wav(backwards(mystery), 'reversed_mystery.wav')

    # synth = load_wav('sounds/synth.wav')
    # water = load_wav('sounds/water.wav')
    # write_wav(mix(synth, water, 0.2), 'mixed_synth_n_water.wav')

    # chord = load_wav('sounds/chord.wav')
    # write_wav(echo(chord, 5, 0.3, 0.6), 'echoy_chord.wav')

    car = load_wav("sounds/car.wav", stereo=True)
    write_wav(pan(car), "l2r_car.wav")

    mountain = load_wav("sounds/lookout_mountain.wav", stereo=True)
    write_wav(remove_vocals(mountain), "mountain_mono.wav")
